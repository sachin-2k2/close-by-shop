...
//   const Product_Complaint({required this.orderid,required this.productIds ,super.key});
...
//                               child: ListTile(
//                                 title: Text(
//                                   replay[index]['complaint'] ?? 'complaint',
...
class _Product_ComplaintState extends State<Product_Complaint> {
    ...
    try {
      ...
      final response = await dio.post(
        ...
        data: {
          ...
          'orderid': widget.orderid,
          'complaint': comp.text,
          'PRODUCTID': widget.productIds.join(','), // Send multiple IDs as comma-separated
        },
      );
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        comp.clear();
        await get_replay(context); // Refresh all complaints
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to submit complaint')),
        );
  ...
  Future<void> get_replay(context) async {
    try {
      ...
      final response = await dio.get('$baseurl/complaintapi/$loginid');
      print("Response: ${response.data}");

      if (response.statusCode == 200 || response.statusCode == 201) {
        setState(() {
          // Extract all complaints from response
          replay = response.data['complaints'] ?? [];
        });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to fetch complaint')),
      ...
      print("Error fetching replay: $e");
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Error fetching complaints')),
      );
    }
  }
                        ...
                        ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return Card(
                              child: ListTile(
                                title: Text(
                                  replay[index]['complaint'] ?? 'No complaint',
                                  style: const TextStyle(
                                    fontWeight: FontWeight.bold,
                                  ),
                                ),
